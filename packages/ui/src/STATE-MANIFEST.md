...
| eyebrow   | stable | —                                                            | —            | #44 | 3        | `packages/ui/src/components/eyebrow/Eyebrow.rfc.md` | Third component candidate: the site's one typographic mono label ("eyebrow" above a section title, "tag" inside a card, a row or a sigil — usage analysis `process/archives/2026-09-16-mono-label-usage-analysis.md`). RFC approved 2026-09-17 (§7 Q12) and implemented the same day (PR #40) — contract, meta, a11y suite, docs and dist. The first component with **no Figma docs page**: its rules are arbitrated in session and recorded in RFC §7. The Callout's tag shares the recipe but stays inside the Callout (no dependency in v1). Priority 3 set by the design lead 2026-09-17 (RFC §7 Q11); eight closed questions arbitrated in session 2026-09-16/17 (§7 Q4–Q11). **Default tone `dim` replaces `mute`** 2026-09-21 (§7 Q13): at the site adoption's visual checkpoint (site `main` 4ed5b3d, dist pin c17e443, 28 usages) the design lead judged `fg/mute` too dark; Figma variant 80:3 is `Tone=Dim`, bound to `fg/dim`. **Adopted by LearningLeagues** 2026-09-21 (site `main` 64193d7, dist pin bb039ef): 28 label usages replaced plus 3 showcase specimens, 13 local class recipes deleted, layout kept product-side (`.label-slot`, `.sigil-label`, `.build-head`), the two in-heading labels moved before their h3 (§7 Q9; two were found in source, not three), zero compensations; checkpoint decisions in §7 Q14, ratification in §7 Q15. Promoted `stable` 2026-09-21 by `promote.ts` (PR #43): a11y `pass` by the accessibility engine gate. Promoted `exported` 2026-09-21 (PR #44, the design lead's call). No session owns it. | —               |
| sigil     | draft  | Sigil implementation session (Claude, sup. Louis Tinthilier) | `feat/sigil` | #48 | 4        | `packages/ui/src/components/sigil/Sigil.rfc.md`     | Fourth component candidate: the framed square the site's heroes carry (art in the live accent, two inset rings; the corner label left the design at the Figma checkpoint, RFC §7 Q11) — usage analysis `process/archives/2026-09-26-sigil-usage-analysis.md` (5 mounts on 9 pages under 3 recipes), design record `process/archives/2026-09-26-sigil-design.md` (12 questions arbitrated in session 2026-09-26, two of them at the design lead's Figma checkpoint: no label, art inside the accent ring). No library dependency. Priority 4 set by the design lead 2026-09-26. RFC approved 2026-09-26 (§7 Q13, after the design lead's own Figma edit); no Figma docs page (page-less path, rules in RFC §7). **Implemented 2026-09-26 (PR #48)**: component, css, five-test suite, docs and dist; token `Primitives/opacity/ornament` extracted (checksum-verified) with the transform's opacity percent rule; tooling grown on the way — element children in contract examples, the docs-smoke gate counts drawn SVG marks, the tokens page renders non-colour primitives. Contract `draft` / a11y `pending` until `pnpm promote`. Next: site adoption (5 mounts, 9 pages, captions removed — §7 Q12), then promotion.                                                                                                                                                                                                                                                                                                                                      | —               |
