...
The one ARIA attribute the component emits is `aria-current="page"` when
`current` is set — valid on both elements, and the filled visual keys on it
(§3.1), so the state cannot be shown without being announced.
...
| 2026-08-31 | (adoption pilot) Le nav-cta « Role quiz » du header, exclu du composant par §2.3, "est différent de tous" — le migrer vers Button ? Quelle variante, et son état rempli `is-active` (+ `aria-current="page"`) que Button ne porte pas ?                                                                                                                                                                                                                                                                | "Secondary (Recommended)" ; "Abandonner cet état (Recommended)" — the header CTA becomes the library Button; the filled you-are-here state AND its `aria-current` semantic are dropped, recorded as the first product→library ask: a selected/aria-current state on Button, to return via RFC. "← Back to site" stays product chrome                                                                                                                              | Louis Tinthilier |
| 2026-08-31 | Les deux « Role quiz » ne sont pas iso : "L'un a un icone devant l'autre derrière."                                                                                                                                                                                                                                                                                                                                                                                                                    | "Label + flèche → derrière (Recommended)" — both align on the site's dominant pattern (label then trailing SVG arrow); the hero's leading quiz pictogram and the header's text arrow retired                                                                                                                                                                                                                                                                      | Louis Tinthilier |
| 2026-08-31 | "Pourquoi y a pas les icones dans le composant que t'as créé sur Figma ?" — the 2026-08-14 icon-slot deferral's own clause is triggered: the pilot measured 9/10 rendered buttons carrying a trailing glyph.                                                                                                                                                                                                                                                                                           | "go icone" — the Figma set gains an `Icon ?` boolean (trailing arrow, default Yes, stroke bound to each variant's label variable). Code unchanged: icons remain inline-SVG children (§3.2); no `icon` prop until a real need constrains the glyphs                                                                                                                                                                                                                | Louis Tinthilier |
| 2026-09-16 | (follow-up of the 2026-08-31 filed ask) Four closed questions on the selected / aria-current state: (1) API — prop `current` → aria-current="page" on either rendering, CSS keyed on the attribute / prop `selected` + class / `current` limited to `<a href>`; (2) visual — secondary only, the site's `.nav-cta.is-active` recipe / both variants; (3) Figma — the variant only, now / variant + refresh Docs / Button / no write; (4) site adoption in this session after the merge / library only. | (1) "Prop `current` → aria-current=\"page\" (Recommended)"; (2) "Secondary seul, recette du site (Recommended)" — primary + current emits the attribute with no visual (§5); (3) "Variante seule, maintenant (Recommended)" — variant `Style=Secondary, State=Current` written (node 73:2); `Docs / Button` (54:3) lags by one state until the next Button write; (4) "Oui, après le merge lib (Recommended)" — the push to the product repository stays Louis's. | Louis Tinthilier |
| 2026-09-16 | (visual checkpoint of the delivered state) "Qaund tu dis current tu veux dire pressed ?" — answered: no, current = you-are-here (`aria-current="page"`, navigation), pressed = a toggle (`aria-pressed`), out of scope. Then: "Ok mais on ne s'en servira quasiment nul part je suis pas sûr de l'utilité. Tu vois un usecase ?" — one measured usage (the header CTA on the Quiz page, the state the site lost at adoption); ship or drop?                                                            | "Livrer (Recommended)" — shipped as an additive prop; the single known usage is recorded here so the next reader does not mistake it for a widely used state. Same checkpoint: the unbound white fill on the seven variants' icon frames (pre-existing `createAutoLayout` artefact) cleared in Figma, "Vider le fill sur les 7 variantes (Recommended)".                                                                                                          | Louis Tinthilier |
