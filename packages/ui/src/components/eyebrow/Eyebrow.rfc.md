...
> **Date:** 2026-09-16
> **Design node:** `80:7` — [link](https://www.figma.com/design/6zp7CvEjdiFXzwh6ZGGwB8/Lib?node-id=80-7) (component set `Eyebrow`, page `Eyebrow` 80:2: variants `Tone=Mute` 80:3 and `Tone=Accent` 80:5)
...
muted by default or in the live accent, that the site sets above a section
...
Mute and Accent are tones, not states: the component has no interactive
...
| -------- | -------------------- | -------- | -------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| tone     | `"mute" \| "accent"` | `"mute"` | no       | Text colour: `fg/mute` (default, §7 Q3) or `accent/default`, which follows the accent axis (`[data-accent]`). Emits `ll-eyebrow--{tone}`. No other tone in v1 (§7 Q3). TEST |
...
- `tone` omitted → `ll-eyebrow--mute`; `tone="accent"` → `ll-eyebrow--accent`.
  TEST
...
- Every value in `eyebrow.css` is a token: the `type/meta` family
  (`--ll-type-meta-*`) plus `text-transform: uppercase`; colour `--ll-fg-mute`
...
- Contrast of `fg/mute` and of `accent/default` under the five accents,
...
- Contrast is computed by the gate from resolved tokens, never declared here:
  `fg/mute` on `surface/default` and on `bg` (the label sits on both — section
