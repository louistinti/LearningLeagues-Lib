...
- Figma set 16:18: variants `Style=Primary|Secondary × State=Default|Hover|Focus` (six components at x=24, y=24+64·i), boolean `Icon ?#43:0`, text prop `Label#16:0`. `Style=Secondary, State=Default` (16:10, at y=216): no fill, stroke bound to `border/default` (1px), horizontal auto-layout 22/12 padding, gap 10; label TEXT 16:11 bound to `fg/default` (JetBrains Mono Bold, text style `type/button`); icon FRAME 43:8 (14×14) holding VECTOR 43:9 with stroke bound to `fg/default`. Pre-existing: the icon frame carried an unbound white fill (the `createAutoLayout` default) on every variant — flagged at the visual checkpoint and cleared there with Louis's approval (see §3).
