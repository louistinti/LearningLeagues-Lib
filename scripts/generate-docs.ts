...
for (const slug of slugs) {
  ...
  if (!metaFile) {
    // RFC stage (arbitrated 2026-09-11, ORCHESTRATION "You created an RFC-stage
    // component folder"): contract.json without a *.meta.ts is a component
    // whose RFC is being written — not shippable surface, so it is left out
    // of the site, visibly. A folder WITH a meta.ts must be complete.
    rfcStage.push(slug);
    continue;
  }
...
for (const slug of rfcStage)
  console.log(`generate-docs: skipped ${slug} (RFC stage — contract.json without *.meta.ts)`);
