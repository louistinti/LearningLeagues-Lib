...
import { childSnippet, type ExampleChildren } from "./example-children.ts";
...
export interface ComponentDoc {
  ...
  meta: {
    ...
    notes?: string;
    examples: { label: string; props: Record<string, unknown>; children?: ExampleChildren }[];
    guidelines?: {
...
// JSX source of a contract example, shown under its demo stage: string
// children are derived from the example; the element form prints its
// hand-written snippet (scripts/lib/example-children.ts).
function jsxSource(name: string, e: ComponentDoc["meta"]["examples"][number]): string {
  const attrs = Object.entries(e.props)
    ...
    .join("");
  const snippet = childSnippet(e.children);
  return snippet !== undefined ? `<${name}${attrs}>${snippet}</${name}>` : `<${name}${attrs} />`;
}
